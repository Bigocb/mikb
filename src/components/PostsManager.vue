        ...
        <div class="list-group">
          <v-client-table :data="posts" :columns="columns">
            ...
            <div slot="title" slot-scope="props">
             <router-link v-on:click.native="updateReadCount(props.row.id)" :to="'/detail/' + props.row.id">{{props.row.title}}</router-link>
            </div>
            <div slot="category" slot-scope="props">
              <span class="tag" v-for="tag in splitJoin(props.row.tags)" v-text="tag"></span>
            </div>
...
<script>
  ...
  export default {
    data() {
      ...
      return {
        sortKey: '',
        columns: ['title','category','date','delete'],
         options: {
